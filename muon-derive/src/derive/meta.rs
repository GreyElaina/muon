...
impl ObserveMeta {
        ...
        match arg.ident.to_string().as_str() {
            ...
            "shallow" => {
                self.general_impl = Some(GeneralImpl {
                    ...
                    // `SerializeSnapshot` implies `Snapshot` with a
                    // serializable associated snapshot.
                    bounds: parse_quote! { ::muon::general::SerializeSnapshot + ::serde::Serialize },
            ...
            "derive" => {
                if attribute_kind != AttributeKind::Item {
                    errors.extend(
                        syn::Error::new(
                            arg.ident.span(),
                            "the 'derive' argument is only allowed on items",
                        )
                        .to_compile_error(),
                    );
                }
            ...
            "expose" => {
                if attribute_kind != AttributeKind::Item {
                    errors.extend(
                        syn::Error::new(
                            arg.ident.span(),
                            "the 'expose' argument is only allowed on items",
                        )
                        .to_compile_error(),
                    );
                }
    ...
    fn parse_serde(&mut self, arg: MetaArgument) {
        match (
            arg.ident.to_string().as_str(),
            arg.value.map(|(_, expr)| expr),
        ) {
