...
pub fn derive_observe(mut input: syn::DeriveInput) -> TokenStream {
    ...
    if input_meta.general_impl.is_none()
        ...
        && fields.iter().all(|field| {
            let meta = ObserveMeta::parse_attrs(
                &field.attrs,
                &mut errors,
                AttributeKind::Field,
                DeriveKind::Struct,
        ...
        return quote! {
            ...
            impl #impl_generics ::muon::Observe for #input_ident #type_generics where #where_predicates {
                type Observer<#ob_lt, #head, #depth>
                    = ::muon::general::#ob_ident<#ob_lt, Self, #head, #depth>
                where
                    Self: #ob_lt,
    ...
    match &input.data {
        ...
        _ => syn::Error::new(
            input.span(),
            "Observe can only be derived for structs or enums",
        )
        .to_compile_error(),
    }
}
...
impl<'i, 'ast: 'i> Visit<'ast> for GenericsVisitor<'i> {
    ...
    fn visit_lifetime_param(&mut self, lt_param: &'ast syn::LifetimeParam) {
        self.lt_idents
            .insert(Cow::Borrowed(&lt_param.lifetime.ident));
    }
}
...
/// Resolves the element observer for a field type: the observer of the
/// field type's last type parameter (e.g. `CrdtVec<Block>` -> the
/// observer of `Block`). Types without type parameters (plain values,
/// derived models) fall back to the field observer itself.
pub(crate) fn element_observer(
    ob_lt: &syn::Lifetime,
    field_ty: &syn::Type,
    fallback: &TokenStream,
) -> TokenStream {
    let mut last: Option<&syn::Type> = None;
    ...
    match last {
        Some(ty) => quote! { ::muon::observe::DefaultObserver<#ob_lt, #ty> },
        None => fallback.clone(),
    }
}
