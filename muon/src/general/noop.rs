...
impl<T: ?Sized, S: Sink + ?Sized> SerializeObserverState<T, S> for NoopObserverState<T> {
    fn flush(&mut self, _: &T, _: &mut S) {}
}

shallow_observer! {
    /// A general observer that never reports changes.
    ///
    /// [`NoopObserver`] is a no-operation [`Observer`](crate::observe::Observer): it never reports
    /// events to the sink, effectively ignoring all mutations to the observed value.
    ///
    /// ## Derive Usage
    ///
