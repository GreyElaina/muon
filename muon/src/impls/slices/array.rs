    ...
    O: QuasiObserver<InnerDepth = Zero, Head: Sized>,
{
    ...
    /// (producing a full [`Replace`](crate::Changed::Replace)), the array implementation
    ...
    /// comparable in size to a whole-array [`Replace`](crate::Changed::Replace), while
...
{
    fn flush<Sk: Sink + ?Sized>(&mut self, _ptr: &mut Pointer<S>, sink: &mut Sk)
    where
        O: Flush<Sk>,
    {
        for (index, ob) in self.iter_mut().enumerate() {
            sink.push_index(index);
            <O as Flush<Sk>>::flush(ob, sink);
            sink.pop_segment();
        }
    }
}
...
impl<const N: usize, O, S: ?Sized, D, Sk: Sink + ?Sized> QuasiSink<Sk>
    for ArrayObserver<N, O, S, D>
{
    type Operation = Sk::Operation;
    type Identity = Sk::Identity;
}

impl<const N: usize, O, S: ?Sized, D, T, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem>
    for ArrayObserver<N, O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = [T; N]>,
...
impl<const N: usize, O, S: ?Sized, D, T, Sk: Sink + ?Sized> Flush<Sk> for ArrayObserver<N, O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = [T; N]>,
    O: Observer<InnerDepth = Zero, Head = T> + Flush<Sk>,
...
impl<const N: usize, O1, O2, S1: ?Sized, S2: ?Sized, D1, D2>
    PartialOrd<ArrayObserver<N, O2, S2, D2>> for ArrayObserver<N, O1, S1, D1>
where
    D1: Unsigned,
    D2: Unsigned,
    O1: Observer<InnerDepth = Zero, Head: Sized>,
...
{
    fn flush<S: Sink + ?Sized>(&self, snapshot: [T::Snapshot; N], sink: &mut S) {
        for (v, s) in self.iter().zip(snapshot) {
...
mod tests {
    ...
    fn index_by_usize() {
        ...
        *ob[1].tracked_mut() = 99;
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [1], "before": 20, "after": 99}]),
        );
    }
    ...
    fn multiple_index_mutations() {
        ...
        *ob[2].tracked_mut() = 30;
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [0], "before": 1, "after": 10},
                {"path": [2], "before": 3, "after": 30},
            ]),
        );
    }
    ...
    fn deref_mut_triggers_replace() {
        ...
        *ob.tracked_mut() = [4, 5, 6];
        let changes = __flush!(&mut ob);
        // DerefMut on array invalidates every element, yielding per-element
        // changes with positive indices.
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [0], "before": 1, "after": 4},
                {"path": [1], "before": 2, "after": 5},
                {"path": [2], "before": 3, "after": 6},
            ]),
        );
    }
    ...
    fn swap() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [0], "before": 10, "after": 30},
                {"path": [2], "before": 30, "after": 10},
            ]),
        );
    }
    ...
    fn nested_string_append() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [0], "before": "hello", "after": "hello!"}]),
        );
    }
    ...
    fn flush_resets_state() {
        ...
        let changes = __flush!(&mut ob);
        assert!(!changes.is_empty());
        // Second flush with no new changes returns empty.
        let changes = __flush!(&mut ob);
        assert!(changes.is_empty());
    }
    ...
    fn observe_provenance_write() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [0], "before": "hello", "after": "hello!"},
                {"path": [1], "before": "world", "after": "world?"},
            ]),
        );
    }
}
