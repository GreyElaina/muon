...
use crate::impls::slice::{
    LazyVec, SliceObserver, SliceObserverState, SliceSerializeObserverState,
};
...
/// appended and truncated boundaries.
///
...
/// report [`Replace`](crate::Changed::Replace) and there was at least some tracked content
/// (`append_index > 0` or `truncate_len > 0`), the granular mutations are collapsed into a single
/// whole-slice [`Replace`](crate::Changed::Replace).
pub struct VecObserverState<O> {
    ...
    append_index: usize,
    /// Pre-write snapshot of the whole slice, captured at observe time
    /// and refreshed at every flush. Serves as the `Replace.before` of
    /// a whole-slice replace (element removal).
    ...
    O::Head: SerializeSnapshot,
{
    ...
    unsafe fn observe(target: *mut Self::Target) -> Self {
        ...
        Self {
            ...
            snapshot: Some(serde_json::to_value(slice.to_snapshot()).expect("snapshot serializes")),
    ...
    O::Head: SerializeSnapshot + Sized + 'static,
{
        ...
        O: Flush<Sk>,
    {
        ...
        if truncate_len > 0 {
            // Elements were removed: the removed before-values are not
            // recoverable per element, so fall back to a whole-slice
            // replace (the pre-write snapshot is the `before`).
            self.inner.get_mut().truncate(0);
            let before = self.snapshot.take();
            let after = Some(serde_json::to_value(slice).expect("serialization cannot fail"));
            self.snapshot =
                Some(serde_json::to_value(slice.to_snapshot()).expect("snapshot serializes"));
            sink.replace(
        ...
        for range in inner.initialized.overlapping(&(0..append_index)) {
                ...
                .take(end - range.start)
            {
                unsafe {
                    Observer::relocate(
                        inner.data[i].assume_init_mut(),
                        head as *const O::Head as *mut O::Head,
                    )
                };
            }
        }

        // Appended region: no before value.
        for (i, item) in slice[append_index..].iter().enumerate() {
            sink.push_neg_index(slice.len() - append_index - i);
        ...
        // (their creation-time snapshot). The observer slots for the
        // existing region stay untouched.
        inner.truncate(append_index);

        // Flush initialized observers in reverse, prefixing each with its
        // position from the tail.
        for range in inner.initialized.overlapping(&(0..append_index)).rev() {
            let end = range.end.min(append_index);
            for i in (range.start..end).rev() {
                sink.push_neg_index(slice.len() - i);
                <O as Flush<Sk>>::flush(unsafe { inner.data[i].assume_init_mut() }, sink);
                sink.pop_segment();
            }
        }
        // Refresh the whole-slice snapshot so a later wholesale replace
...
impl<O, S: ?Sized, D, Sk: Sink + ?Sized> QuasiSink<Sk> for VecObserver<O, S, D> {
    type Operation = Sk::Operation;
    type Identity = Sk::Identity;
}

impl<O, S: ?Sized, D, T, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem>
    for VecObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = Vec<T>>,
    O: Observer<InnerDepth = Zero, Head = T> + Flush<Sk>,
    T: SerializeSnapshot + 'static,
{
...
impl<O, S: ?Sized, D, T, Sk: Sink + ?Sized> Flush<Sk> for VecObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = Vec<T>>,
    O: Observer<InnerDepth = Zero, Head = T> + Flush<Sk>,
    ...
    T: SerializeSnapshot,
{
    ...
    pub fn extract_if<F, R>(
        ...
        range: R,
        mut filter: F,
    ) -> ExtractIf<'_, T, impl FnMut(&mut T) -> bool>
    where
        F: FnMut(&mut T) -> bool,
        R: RangeBounds<usize>,
    {
    ...
    O: Observer<InnerDepth = Zero, Head: Sized + Debug>,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("VecObserver")
            .field(&self.untracked_ref())
            .finish()
    }
}

impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialEq<VecObserver<O2, S2, D2>>
    for VecObserver<O1, S1, D1>
where
    D1: Unsigned,
    D2: Unsigned,
...
impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialOrd<VecObserver<O2, S2, D2>>
    for VecObserver<O1, S1, D1>
where
    D1: Unsigned,
    D2: Unsigned,
...
impl<T: Observe + SerializeSnapshot> Observe for Vec<T> {
    ...
    Self::Snapshot: serde::Serialize + 'static,
{
    fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
        SerializeSnapshot::flush(&self.as_slice(), snapshot, sink)
    }
}
...
mod tests {
    ...
    fn extend_from_slice() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [-2], "before": null, "after": 6},
                {"path": [-1], "before": null, "after": 7},
            ]),
        );
    }
    ...
    fn append_and_index() {
        ...
        *ob[1].tracked_mut() = 12;
        let changes = __flush!(&mut ob);
        // The appended element is reported first (its current value is 12),
        // then the existing element 0.
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [-1], "before": null, "after": 12},
                {"path": [-2], "before": 1, "after": 11},
            ]),
        );
    }
    ...
    fn non_replace_child_with_append() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [-1], "before": null, "after": "new"},
                {"path": [-3], "before": "hello", "after": "hello!"},
            ]),
        );
    }
    ...
    fn index_by_range() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([
                {"path": [-2], "before": 3, "after": 333},
                {"path": [-3], "before": 2, "after": 222},
            ]),
        )
    }
    ...
    fn pop_push_clears_stale_state() {
        ...
        let changes = __flush!(&mut ob);
        assert!(!changes.is_empty()); // whole-vec Replace (truncate + append)
    ...
    fn swap_remove_triggers_truncate() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": [1, 2, 3, 4], "after": [1, 4, 3]}]),
        );
    }
    ...
    fn drain_range_triggers_truncate() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": [1, 2, 3, 4], "after": [1, 4]}]),
        );
    }
    ...
    fn splice_triggers_truncate() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": [1, 2, 3, 4], "after": [1, 10, 20, 30, 4]}]),
        );
    }
    ...
    fn extract_if_preserves_prefix() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": [1, 2, 3, 4, 5], "after": [1, 2, 3, 4]}]),
        );
    }
    ...
    fn extract_if_straddles_append_index() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": [1, 2, 3], "after": [1, 3, 5]}]),
        );
    }
    ...
    fn push_mut_on_empty_triggers_replace() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [-1], "before": null, "after": "xy"}]),
        );
    }
    ...
    fn insert_mut_at_end_acts_like_push() {
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [-1], "before": null, "after": "d!"}]),
        );
    }
    #[test]
