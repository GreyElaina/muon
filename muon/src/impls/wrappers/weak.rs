use std::fmt::Debug;
use std::marker::PhantomData;
use std::ops::{Deref, DerefMut};

use serde::Serialize;

use crate::general::{SerializeSnapshot, Snapshot};
use crate::helper::macros::{spec_impl_observe_from_ro, spec_impl_ro_observe};
use crate::helper::{AsDeref, AsDerefMut, Pointer, QuasiObserver, Succ, Unsigned, Zero};
use crate::observe::{Flush, FlushWith, Observer, QuasiSink, Sink};

trait Weak<T: ?Sized> {
    type Ptr: Deref<Target = T>;

    fn upgrade(&self) -> Option<Self::Ptr>;
}

impl<T: ?Sized> Weak<T> for std::rc::Weak<T> {
    type Ptr = std::rc::Rc<T>;

    fn upgrade(&self) -> Option<Self::Ptr> {
        self.upgrade()
    }
}

impl<T: ?Sized> Weak<T> for std::sync::Weak<T> {
    type Ptr = std::sync::Arc<T>;

    fn upgrade(&self) -> Option<Self::Ptr> {
        self.upgrade()
    }
}

/// Observer implementation for [`std::rc::Weak<T>`] and [`std::sync::Weak<T>`].
pub struct WeakObserver<O, S: ?Sized, D> {
    ptr: Pointer<S>,
    mutated: bool,
    initial: bool,
    inner: Option<O>,
    /// Pre-write snapshot of the whole weak reference, captured at
    /// observe time and refreshed at every flush.
    snapshot: Option<serde_json::Value>,
    phantom: PhantomData<D>,
}

impl<O, S: ?Sized, D> Deref for WeakObserver<O, S, D> {
    type Target = Pointer<S>;

    fn deref(&self) -> &Self::Target {
        &self.ptr
    }
}

impl<O, S: ?Sized, D> DerefMut for WeakObserver<O, S, D> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.mutated = true;
        self.inner = None;
        &mut self.ptr
    }
}

impl<O, S: ?Sized, D> QuasiObserver for WeakObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D>,
{
    type Head = S;
    type OuterDepth = Succ<Zero>;
    type InnerDepth = D;

    fn invalidate(this: &mut Self) {
        this.mutated = true;
        this.inner = None;
    }
}

impl<O, S: ?Sized, D> Observer for WeakObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target: Weak<O::Head>>,
    O: Observer<InnerDepth = Zero>,
    O::Head: SerializeSnapshot,
{
    unsafe fn observe(head: *mut Self::Head) -> Self {
        unsafe {
            let rc = (&*head).as_deref().upgrade();
            Self {
                mutated: false,
                initial: rc.is_some(),
                snapshot: Some(
                    serde_json::to_value(rc.as_deref().to_snapshot()).expect("snapshot serializes"),
                ),
                inner: rc.map(|ptr| O::observe(std::ptr::from_ref(&*ptr).cast_mut())),
                ptr: Pointer::new_unchecked(head),
                phantom: PhantomData,
            }
        }
    }

    unsafe fn relocate(this: &mut Self, head: *mut Self::Head) {
        unsafe {
            if let Some(inner) = &mut this.inner
                && let Some(ptr) = (&*head).as_deref().upgrade()
            {
                O::relocate(inner, std::ptr::from_ref(&*ptr).cast_mut());
            }
            Pointer::set_unchecked(&this.ptr, head);
        }
    }
}

impl<O, S: ?Sized, D, Sk: Sink + ?Sized> QuasiSink<Sk> for WeakObserver<O, S, D> {
    type Operation = Sk::Operation;
    type Identity = Sk::Identity;
}

impl<O, S: ?Sized, D, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem> for WeakObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target: Weak<O::Head>>,
    O: Observer<InnerDepth = Zero> + Flush<Sk>,
    O::Head: SerializeSnapshot + 'static,
    <S::Target as Weak<O::Head>>::Ptr: Serialize,
{
    fn flush_with<F>(this: &mut Self, sink: &mut Sk, _flush_elem: F)
    where
        F: FnMut(&mut Elem, &mut Sk),
    {
        <Self as Flush<Sk>>::flush(this, sink)
    }
}

impl<O, S: ?Sized, D, Sk: Sink + ?Sized> Flush<Sk> for WeakObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target: Weak<O::Head>>,
    O: Observer<InnerDepth = Zero> + Flush<Sk>,
    O::Head: SerializeSnapshot + 'static,
    <S::Target as Weak<O::Head>>::Ptr: Serialize,
{
    fn flush(this: &mut Self, sink: &mut Sk) {
        let value = (*this.ptr).as_deref();
        let rc = value.upgrade();
        let initial = this.initial;
        this.initial = rc.is_some();
        if !this.mutated {
            match (&mut this.inner, rc.is_some()) {
                // Target alive: flush the element observer (its
                // pointer was fixed at observe/relocate time).
                (Some(ob), true) => return <O as Flush<Sk>>::flush(ob, sink),
                // Target released: the element observer dangles — drop
                // it and report the alive -> dead transition. A weak
                // reference never becomes alive again without a write
                // (which sets `mutated`), so this is terminal.
                (Some(_), false) => {
                    this.inner = None;
                    if initial {
                        let before = this.snapshot.take();
                        this.snapshot = None;
                        sink.replace(
                            before.as_ref().map(|v| v as &dyn erased_serde::Serialize),
                            None,
                        );
                    }
                    return;
                }
                // No element observer: the target was never alive
                // during observation, nothing to report.
                (None, _) => return,
            }
        }
        this.mutated = false;
        if initial || rc.is_some() {
            let before = this.snapshot.take();
            let after = rc
                .as_deref()
                .map(|v| serde_json::to_value(v).expect("serialization cannot fail"));
            this.snapshot = Some(
                serde_json::to_value(rc.as_deref().map(|v| v.to_snapshot()))
                    .expect("snapshot serializes"),
            );
            sink.replace(
                before.as_ref().map(|v| v as &dyn erased_serde::Serialize),
                after.as_ref().map(|v| v as &dyn erased_serde::Serialize),
            )
        }
    }
}

impl<O, S: ?Sized, D> Debug for WeakObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D>,
    S::Target: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("WeakObserver")
            .field(&self.untracked_ref())
            .finish()
    }
}

spec_impl_observe_from_ro!(
    WeakObserveImpl,
    std::rc::Weak<Self>,
    std::rc::Weak<T>,
    WeakObserver
);
spec_impl_ro_observe!(
    WeakRoObserveImpl,
    std::rc::Weak<Self>,
    std::rc::Weak<T>,
    WeakObserver
);

spec_impl_observe_from_ro!(
    SyncWeakObserveImpl,
    std::sync::Weak<Self>,
    std::sync::Weak<T>,
    WeakObserver,
);

spec_impl_ro_observe!(
    SyncWeakRoObserveImpl,
    std::sync::Weak<Self>,
    std::sync::Weak<T>,
    WeakObserver,
);

impl<T: Snapshot + ?Sized> Snapshot for std::rc::Weak<T> {
    type Snapshot = Option<T::Snapshot>;

    fn to_snapshot(&self) -> Self::Snapshot {
        self.upgrade().map(|v| v.to_snapshot())
    }
}

impl<T: SerializeSnapshot + ?Sized> SerializeSnapshot for std::rc::Weak<T>
where
    Self::Snapshot: serde::Serialize + 'static,
{
    fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
        SerializeSnapshot::flush(&self.upgrade(), snapshot, sink)
    }
}

impl<T: Snapshot + ?Sized> Snapshot for std::sync::Weak<T> {
    type Snapshot = Option<T::Snapshot>;

    fn to_snapshot(&self) -> Self::Snapshot {
        self.upgrade().map(|v| v.to_snapshot())
    }
}

impl<T: SerializeSnapshot + ?Sized> SerializeSnapshot for std::sync::Weak<T>
where
    Self::Snapshot: serde::Serialize + 'static,
{
    fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
        SerializeSnapshot::flush(&self.upgrade(), snapshot, sink)
    }
}
