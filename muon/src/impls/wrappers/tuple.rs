...
impl<O, S: ?Sized, D, Sk: Sink + ?Sized> Flush<Sk> for TupleObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = (O::Head,)>,
    O: Observer<InnerDepth = Zero> + Flush<Sk>,
    O::Head: Serialize + Sized + 'static,
{
    fn flush(this: &mut Self, sink: &mut Sk) {
        sink.push_index(0);
        <O as Flush<Sk>>::flush(&mut this.0, sink);
        sink.pop_segment();
    }
}
    ...
    S::Target: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("TupleObserver")
            .field(&self.untracked_ref())
            .finish()
    }
}

impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialEq<TupleObserver<O2, S2, D2>>
    for TupleObserver<O1, S1, D1>
where
...
impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialOrd<TupleObserver<O2, S2, D2>>
    for TupleObserver<O1, S1, D1>
where
    O1: QuasiObserver<Target: Deref<Target: AsDeref<O1::InnerDepth>>>,
    O2: QuasiObserver<Target: Deref<Target: AsDeref<O2::InnerDepth>>>,
    ...
    Self::Snapshot: serde::Serialize + 'static,
{
    fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
        sink.push_index(0);
        SerializeSnapshot::flush(&self.0, snapshot.0, sink);
        sink.pop_segment();
    }
}
...
macro_rules! tuple_observer {
    ($ty:ident; $ptr:tt; $($o:ident, $p:ident, $t:ident, $u:ident, $n:tt);*) => {
        ...
        impl<$($o,)* S: ?Sized, D, Sk: Sink + ?Sized> QuasiSink<Sk> for $ty<$($o,)* S, D> {
            type Operation = Sk::Operation;
            type Identity = Sk::Identity;

        }

        impl<$($o,)* S: ?Sized, D, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem> for $ty<$($o,)* S, D>
        where
            D: Unsigned,
            S: AsDeref<D, Target = ($($o::Head,)*)>,
            $($o: Observer<InnerDepth = Zero, Head: Serialize + Sized + 'static> + Flush<Sk>,)*
        {
            fn flush_with<F>(this: &mut Self, sink: &mut Sk, _flush_elem: F)
            where
                F: FnMut(&mut Elem, &mut Sk),
        ...
        impl<$($o,)* S: ?Sized, D, Sk: Sink + ?Sized> Flush<Sk> for $ty<$($o,)* S, D>
        where
            D: Unsigned,
            S: AsDeref<D, Target = ($($o::Head,)*)>,
            $($o: Observer<InnerDepth = Zero, Head: Serialize + Sized + 'static> + Flush<Sk>,)*
        {
            fn flush(this: &mut Self, sink: &mut Sk) {
                $(
                    sink.push_index($n);
                    <$o as Flush<Sk>>::flush(&mut this.$n, sink);
                    sink.pop_segment();
                )*
            }
        }
        ...
        impl<$($t: SerializeSnapshot,)*> SerializeSnapshot for ($($t,)*)
            ...
            fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
                $(
                    sink.push_index($n);
                    SerializeSnapshot::flush(&self.$n, snapshot.$n, sink);
                    sink.pop_segment();
                )*
            }
        }
...
mod tests {
    ...
    fn deref_triggers_replace() {
        ...
        *ob.tracked_mut() = (String::from("world"),);
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [0], "before": "hello", "after": "world"}]),
        );
