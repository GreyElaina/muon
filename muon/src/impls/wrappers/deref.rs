...
use crate::observe::{Flush, FlushWith, Observer, QuasiSink, RoObserve, Sink};
...
impl<O, Sk: Sink + ?Sized> QuasiSink<Sk> for DerefObserver<O> {
    type Operation = Sk::Operation;
    type Identity = Sk::Identity;
}

impl<O, D, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem> for DerefObserver<O>
where
    O: Observer<InnerDepth = Succ<D>> + Flush<Sk>,
    O::Head: AsDeref<D>,
    D: Unsigned,
{
...
impl<O, D, Sk: Sink + ?Sized> Flush<Sk> for DerefObserver<O>
where
    O: Observer<InnerDepth = Succ<D>> + Flush<Sk>,
    O::Head: AsDeref<D>,
    D: Unsigned,
{
    fn flush(this: &mut Self, sink: &mut Sk) {
        <O as Flush<Sk>>::flush(&mut this.inner, sink)
    }
}
...
macro_rules! impl_serialize_snapshot {
    ($(impl $([$($gen:tt)*])? SerializeSnapshot for $ty:ty;)*) => {
        $(
            impl <$($($gen)*)?> SerializeSnapshot for $ty where Self::Snapshot: serde::Serialize + 'static {
...
mod test {
    ...
    fn test_deref_replace() {
        ...
        **ob.tracked_mut() = String::from("42");
        ...
        assert_eq!(
            changes.into_json(),
            json!([{"path": [], "before": "Hello, World!", "after": "42"}]),
        );
    }
