...
impl<O, S: ?Sized, D, Sk: Sink + ?Sized> QuasiSink<Sk> for NewtypeObserver<O, S, D> {
    type Operation = Sk::Operation;
    type Identity = Sk::Identity;
}

impl<O, S: ?Sized, D, Sk: Sink + ?Sized, Elem: ?Sized> FlushWith<Sk, Elem>
    for NewtypeObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target: Newtype<Inner = O::Head>>,
    O: Observer<InnerDepth = Zero, Head: Sized> + Flush<Sk>,
{
...
impl<O, S: ?Sized, D, Sk: Sink + ?Sized> Flush<Sk> for NewtypeObserver<O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target: Newtype<Inner = O::Head>>,
    O: Observer<InnerDepth = Zero, Head: Sized> + Flush<Sk>,
    ...
    S::Target: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("NewtypeObserver")
            .field(&self.untracked_ref())
            .finish()
    }
}

impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialEq<NewtypeObserver<O2, S2, D2>>
    for NewtypeObserver<O1, S1, D1>
where
...
impl<O1, O2, S1: ?Sized, S2: ?Sized, D1, D2> PartialOrd<NewtypeObserver<O2, S2, D2>>
    for NewtypeObserver<O1, S1, D1>
where
    O1: QuasiObserver<Target: Deref<Target: AsDeref<O1::InnerDepth>>>,
    O2: QuasiObserver<Target: Deref<Target: AsDeref<O2::InnerDepth>>>,
    ...
    ($helper:ident, $helper_ref:ident, $wrapper:ident) => {
            ...
            Self::Snapshot: serde::Serialize + 'static,
        {
            fn flush<S: Sink + ?Sized>(&self, snapshot: Self::Snapshot, sink: &mut S) {
                SerializeSnapshot::flush(&self.0, snapshot, sink)
            }
        }
    };
}
