    ...
    AsDerefMutCoinductive<Self::OuterDepth, Target: Deref<Target = Self::Head>>
{
    ...
    /// flush should produce a [`Replace`](crate::Changed::Replace) change.
    ///
    ...
    ///             // expressed as an in-place operation, so we do not trigger full mutation.
    ///             self.untracked_mut().pop()
    ///         } else {
    ///             // Otherwise, we need to treat the pop operation as a full replace.
...
impl<T: ?Sized> QuasiObserver for Pointer<T> {
    ...
    /// registers siblings calls `expose_provenance` on `&mut self`
    ...
    fn invalidate(this: &mut Self) {
        ...
        for &(offset, invalidate) in states {
            let target =
                std::ptr::with_exposed_provenance_mut::<u8>(base_addr.wrapping_add_signed(offset));
...
/// `invalidate` rather than `mark_replace` to avoid coupling with a specific change vocabulary.
///
...
pub trait Invalidate<T: ?Sized> {
    ...
    /// [`Replace`](crate::Changed::Replace) change.
    ///
