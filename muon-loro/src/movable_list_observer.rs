...
use crate::{Context, Error, MovableList};

pub struct MovableListObserver<T, O, S: ?Sized, D = Zero> {
    ptr: Pointer<S>,
    state: State<LoroMovableList, O>,

    marker: PhantomData<(fn(&mut T), D)>,
}

impl<T, O, S: ?Sized, D> Deref for MovableListObserver<T, O, S, D> {
...
impl<T, O, S: ?Sized, D> DerefMut for MovableListObserver<T, O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = MovableList<T>>,
{
...
impl<T, O, S: ?Sized, D> QuasiObserver for MovableListObserver<T, O, S, D>
where
    D: Unsigned,
    S: AsDeref<D, Target = MovableList<T>>,
{
...
unsafe impl<T, O, S: ?Sized, D> Observer for MovableListObserver<T, O, S, D>
where
    D: Unsigned,
    S: AsDerefMut<D, Target = MovableList<T>>,
...
impl<T, O, S: ?Sized, D, ContextType: ?Sized, ItemRoute, ChildRoute, CollectError, Semantic, Tail>
    ...
    for MovableListObserver<T, O, S, D>
where
    D: Unsigned,
    S: AsDerefMut<D, Target = MovableList<T>>,
    O: Observer<Head = T, InnerDepth = Zero>
...
impl<T, ItemRoute> ObserveWith<MovableList<T>, Composite<(ItemRoute,)>> for MovableList<T>
where
    T: Serialize,
    T: ObserveWith<T, ItemRoute>,
{
    type Observer<S, D>
        = MovableListObserver<T, <T as ObserveWith<T, ItemRoute>>::Observer<T, Zero>, S, D>
    where
        D: Unsigned,
        S: AsDerefMut<D, Target = MovableList<T>> + ?Sized;
}

impl<T, O, S: ?Sized, D> MovableListObserver<T, O, S, D>
where
    T: Serialize,
    D: Unsigned,
    S: AsDerefMut<D, Target = MovableList<T>>,
    O: Observer<Head = T, InnerDepth = Zero>,
{
    fn values_mut(&mut self) -> &mut MovableList<T> {
    ...
    pub fn push(&mut self, value: T) {
        ...
        values.0.push(value);
        self.state
            .record(move |list| materialize::insert_movable_list(list, index, encoded?));
    }
    ...
    pub fn set(&mut self, index: usize, value: T) -> T {
        ...
        self.state.record(move |list| {
            list.delete(index, 1)?;
            materialize::insert_movable_list(list, index, encoded?)
        });
        old
    }
    ...
    pub fn mov(&mut self, from: usize, to: usize) {
        ...
        let values = self.values_mut();
        let value = values.0.remove(from);
        values.0.insert(to, value);
        self.state.mov(from, to);
        self.state.record(move |list| {
            list.mov(from, to)?;
            Ok(())
        });
